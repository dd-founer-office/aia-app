...
export interface RenderCarouselSlideOptions {
   ...
   *  into a text column on the left and this photo filling the right
   *  column at full content height, rounded corners, cropped to fill
   *  (never stretched) -- see Slide2Style.imageColumnRatio/imageGap/
...
function drawSlide2Family(
  ...
  // photo fills a fixed-height box on the right (frame.contentTop to
  // frame.contentBottom, independent of startY's vertical balancing --
  // the photo always fills its full available height regardless of how
  // much text there is). Without a photo, text keeps the full width,
  // exactly as before.
  ...
  if (hasImage && familyImage) {
    ...
    const imageY = frame.contentTop;
    const imageH = frame.contentBottom - frame.contentTop;
    if (draw) {
      drawRoundedCoverImage(ctx, familyImage, imageX, imageY, imageW, imageH, frame.contentW * style.slide2.imageRadius);
    }
  }
