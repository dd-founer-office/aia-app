...
export interface Slide2Style {
  ...
  sectionHeadingSize: number;
  bodySize: number;
  /** Fraction of frame.contentW the optional family-photo column takes on
   *  the right, when RenderCarouselSlideOptions.familyImage is set -- the
   *  text column takes the remainder minus imageGap. Ignored when no
   *  photo is set (text uses the full content width, as before). */
  imageColumnRatio: number;
  /** Gutter between the text column and the photo, as a fraction of
   *  frame.contentW. */
  imageGap: number;
  /** Photo corner radius, as a fraction of frame.contentW. */
  imageRadius: number;
}
...
export interface RenderCarouselSlideOptions {
   ...
   *  ignored on every other slide. When present, drawSlide2Family splits
   *  into a text column on the left and this photo on the right, top-
   *  aligned with the text and sized to roughly match the text block's
   *  own height (not always the slide's full height -- see
   *  drawSlide2Family's own comment for why), rounded corners, cropped to
   *  fill (never stretched) -- see Slide2Style.imageColumnRatio/imageGap/
   *  imageRadius. Founder-supplied (generated externally, uploaded via
   *  the workspace sidebar), never fetched or generated by this app. */
...
/** Draws img into the rounded rect (x, y, w, h), cropping (never
 *  stretching) to fill it completely -- same "object-fit: cover" behavior
 *  as drawCircularBadge above, just for a rounded rect instead of a
 *  circle. */
...
function drawSlide2Family(
  ...
  // With a photo, the text wraps into a narrower left column and the
  // photo fills a box on the right, top-aligned with the text (startY --
  // the same vertically-balanced position the text itself starts at) and
  // sized to roughly MATCH the text block's own height, not the slide's
  // full available height. Sizing it to the full height regardless of how
  // much text there is looked lopsided for short scenarios: a big empty
  // gap under two short sentences on the left while the photo kept going
  // all the way to the bottom on the right. Clamped to a sensible minimum
  // (a one-line scenario shouldn't get a sliver of a photo) and to the
  // available space (a long scenario's photo never overflows the frame).
  // Without a photo, text keeps the full width, exactly as before.
  const hasImage = Boolean(familyImage);
  const textW = hasImage ? frame.contentW * (1 - style.slide2.imageColumnRatio - style.slide2.imageGap) : frame.contentW;
  ...
  if (hasImage && familyImage) {
    const gap = frame.contentW * style.slide2.imageGap;
    const imageW = frame.contentW * style.slide2.imageColumnRatio;
    const imageX = frame.contentX + textW + gap;
    const available = frame.contentBottom - startY;
    const textBlockHeight = cursorY - startY;
    const imageH = Math.min(available, Math.max(textBlockHeight, available * 0.45));
    if (draw) {
      drawRoundedCoverImage(ctx, familyImage, imageX, startY, imageW, imageH, frame.contentW * style.slide2.imageRadius);
    }
  }
