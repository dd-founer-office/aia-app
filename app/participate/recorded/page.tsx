...
// CA-014 v1.0 + founder-directed amendment, Step 5 -- Participation
// Recorded. Reads the real, just-written participations/participation_causes
// rows for the current month (not client-side flow state, which may already
// be reset) -- this page renders correctly no matter how it's reached:
// right after confirming, or a later revisit within the same month.
export default async function ParticipationRecordedPage() {
  ...
  const { data: participationCauses } = await supabase
    ...
    .select("allocation_amount, causes(slug)")
    .eq("participation_id", participation.id);

  const allocationBySlug = new Map<CauseId, number>();
  for (const row of participationCauses ?? []) {
    ...
    if (slug) allocationBySlug.set(slug, row.allocation_amount as number);
  }
  const recordedCauses = CAUSES.filter((cause) => allocationBySlug.has(cause.id));
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-5 py-10 text-center">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-lg font-medium text-[var(--color-primary)]">
            ₹{participation.amount as number}
          </p>
            ...
            {recordedCauses.map((cause) => {
              ...
              return (
                ...
                >
                  ...
                  {cause.title} · ₹{allocationBySlug.get(cause.id)}
                </span>
              );
            })}
          </div>
